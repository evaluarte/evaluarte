Public Class Reportar_Salida

    Private Sub Reportar_Salida_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        CARGAR()
    End Sub
    ...
    Sub MODIFICAR()
        ...
        Else
            Dim CMD As New OleDb.OleDbCommand("UPDATE  Reportar_Ingresos SET Fecha_Salida='" & PEPE & "'  WHERE Codigo_Colegio='" & CBOCODIGOSEDE.Text & "'AND Grupo='" & CBOCODIGOGRUPO.Text & "'AND Codigo_Simulacro='" & CBOCODIGOSIMULACRO.Text & "'", CN)
            CN.Open()
            CMD.ExecuteNonQuery()
            CN.Close()
            CARGAR()
